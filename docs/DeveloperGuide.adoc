...
The sort command is implemented to sort the current Address book according to the following possible criteria:
* name
* phone
* address
* email
...
The person Class which implements this interface defines compareTo as:
[source,java]
----
...
The import mechanism is essentially adding multiple contacts to the address book automatically by referencing a xml file. It is implemented by calling add function repeatedly till all the new contacts have been added to the current contact list.

One consideration of this mechanism is that contacts with identical names will not be added as the current address book is regarded as the most updated version and the import feature aims to only facilitate addition of new contacts.
...
[NOTE]
We chose asciidoc over Markdown because asciidoc, although a bit more complex than Markdown, provides more flexibility in formatting. It supports adding of new contacts and will skip known contacts residing in the address book.
...
|`**` |user |know the crash report so that when the addressbook crashes |restore backup
...
|`**` |user |use Facebook to search by email|add my friends to social media easily

|`*` |user |be able to export my addressbook information  |I can import it into another device
...
.  Data files should be supported by previous version of address book (backward compatibility)
.  Data files should be portable to another computer with the same process running
.  Address book should backup every 30 days to aid disaster recovery
.  Query in the address book should be executed promptly
.  Address book should not take too much resources for other applications
