...
The mechanism is as two different versions:
* GUI Version
* CLI Version
...
Considering the drastically different accessible file paths between development and production mode, every Picture includes a jar URL for production mode.
Both of these URLs are updated when a new image is added.
...
==== Add/Delete Picture (GUI Version)

The GUI version includes two buttons available to the user in the PersonPanel when a person is selected.
...
image::addImageActivityDiagram.png[width="500"]

==== Delete Image (CLI Version)
...
image::deleteImageActivityDiagram.png[width="500"]
...
The AutoComplete Class stores a list of base commands that is final.
Additionally, it stores:
* A string array of currently displayed autocomplete phrases which is instantiated to the base commands.
* ArrayList of residents names as strings that is updated upon update in the model
...
The String array of currently displayed phrases is updated upon every update instance of the user input to correctly display the current autocomplete phrases.
...
image::highlightDiagram.png[width="1000"]
...
==== Timestamp
* Updated user stories incording to user stories
...
image::BackupCommandActivityDiagram.png[width="500"]
...
image::invalid_date.png[width:800]

image::invalid_duration.png[width:800]
...
* Allow adding events which span more than one day.
...
==== Other Possible Future Implementations:
* Allow deleting events by `Title` or `Datetime`.
* Allow batch deleting of events to clear old events with a single command.
...
image::switch_tab_sequence_diagram.png[width:800]
...
image::sort_activity_diagram.png[width="400"]
...
The current choice of design prevents unnecessary overhead in implementation, and is straightforward. While it may take a longer time in operation when handlinng large amount of data,
it is understood that Residents population does not vary a lot. Since the number of rooms are fixed due to building design, it is safe to assume that all operations are able to complete within a reasonable amount of time.
...
image::Ui_breakdown.png[width:800]
...
==== Notes on changing styling:
* Please refrain from using `!important` because it induces ambiguity and reduces modularity in the codebase
* Use `ids` instead of `classes` as css selectors.
* If the new UI Styling is drastically different from the current version, consider putting it in a new file and changing the loaded css to that instead of the current `DarkTheme.css`
* Do not use `element.setStyle()` unless there is no way using CSS to achieve the same. Using such methods induces difficulty in tracking the source of the styling making it cumbersome to edit later.

==== Other Possible Future Implementations:
* Change colours to suit color-blind users.
* Use modals(pop-ups) to show command results instead of the ResultBox.

// end::styling[]


// tag::eventbook[]
=== Events and EventBook
...
* By default, the events are sorted by `Title`
* Querying for events may throw a `EventNotFoundException` and adding events may throw a `DuplicateEventException`

==== Other Possible Future Implementations:
* Implement a better sort for `Events`. Although they can be sorted by * `Title`, `Location`, `Datetime` and `Description`, a user would like to see upcoming events first. So, a possible sort criteria would be `Number of days until the Event`. So, the upcoming events would be at the top of the list and the past ones and others much later in the future would be further down.


==== Logic
At the logic level, there are two commands which allow addition and deletion of events:

* Add Event Command: `addevent` or `ae`
* Delete Event Command: `deleteevent` or `de`

Both these commands follow the general implementations of an `Undoable Command`. For more details look at their respective sections in the Developer Guide: link:DeveloperGuide.adoc[addEvent] link:DeveloperGuide.adoc[deleteEvent]
...
==== Other Possible Future Implementations:
* Remove unnecessary details such as `Description` from the `EventCard`.
* Allow `EventCards` to be selectable so that when selected, a Panel with all the details about the events is displayed
